...
# Check whether an object has circular references
...
def remove_circ(obj, visited=None):
    ...
    if isinstance(obj, list):
        ...
        for attr in obj.__dict__.keys():
            setattr(obj, attr, remove_circ(obj.__dict__[attr], visited))
...
def to_json(obj):
    ...
    try:
        pickle.dumps(temp_obj)
    except:
        return to_json({"hasIO"})
    ...
    if inspect.isfunction(temp_obj) or inspect.ismethod(temp_obj):
        ...
        if hasattr(temp_obj, '__dict__'):
            obj_ref = json.loads(jsonp(temp_obj)) | to_json(vars(temp_obj))
